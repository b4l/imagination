...
img_lst = list(Path('data_new/img').glob('*.jpeg'))
labels = [l.stem.split('_')[1] for l in img_lst]
...
logits = DenseNet(x=batch_images, nb_blocks=nb_block, filters=f_dim,class_num=class_num,dropout_rate=dropout_rate).model
...
with tf.compat.v1.Session() as sess:
    ...
    for epoch in range(epoch_start,total_epochs):
        ...
        for step in range(total_batch):
            ...
            _, loss, train_accuracy = sess.run([train, cost,accuracy], feed_dict=train_feed_dict)
            print("Step:", str(global_step), "Loss:", loss, "Training accuracy:", train_accuracy)
            
            global_step += 1

        saver.save(sess, save_path=os.path.join(save_dir,model_name),
                            global_step=epoch)
        ...
        for ti in range(batch_size):
            ...
            label_vector = np.zeros(class_num)
            img_class = test_imgslst[ti_idx[ti]].stem.split('_')[1]
            label_vector[int(label_index[img_class])] = 1
        ...
        if epoch>3:
            ...
            for ti in range(len(test_imgslst)):
                ...
                test_x.append(test_x_tmp)
                label_vector = np.zeros(class_num)
                img_class = test_imgslst[ti].stem.split('_')[1]
                label_vector[int(label_index[img_class])] = 1
                test_y.append(label_vector)
            ...
            print('Epoch total:', '%04d' % (epoch), '/ Accuracy =', accuracy_all/int(len(test_imgslst)))
